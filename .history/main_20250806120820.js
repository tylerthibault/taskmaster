...
class TaskMasterEditorExtension {
	...
	/**
	 * Create the editor extension for live preview
	 */
	createExtension() {
		const { Extension } = require('@codemirror/state');
		const { ViewPlugin, EditorView, Decoration, WidgetType } = require('@codemirror/view');
		const { syntaxTree } = require('@codemirror/language');
		
		class ButtonWidget extends WidgetType {
			constructor(buttonId, currentState, groupId, processor, logger) {
				super();
			...
			toDOM() {
				...
				span.className = 'taskmaster-live-preview-widget';
				
				// Use the same processor logic to create the button
				this.processor.createButton(span, this.buttonId, this.currentState, this.groupId, {
					sourcePath: this.processor.app.workspace.getActiveFile()?.path
				});
			...
			eq(other) {
				return other.buttonId === this.buttonId && 
					   other.currentState === this.currentState &&
					   other.groupId === this.groupId;
			}
		}

		const buttonPlugin = ViewPlugin.fromClass(class {
			constructor(view) {
				this.view = view;
			...
			buildDecorations() {
				const { processor, logger } = this.plugin;
				...
				for (let i = 1; i <= doc.lines; i++) {
					...
					while ((match = regex.exec(line.text)) !== null) {
						...
						const buttonConfig = processor.parseButtonParams(buttonParams);
