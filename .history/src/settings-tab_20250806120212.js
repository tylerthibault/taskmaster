...
import { StateGroupModal } from './modals/state-group-modal.js';
...
export class TaskMasterSettingTab extends PluginSettingTab {
	...
	createGeneralSettings(containerEl) {
		...
		new Setting(containerEl)
			...
			.addDropdown(dropdown => {
				...
				Object.keys(this.plugin.settings.stateGroups).forEach(groupId => {
					const group = this.plugin.settings.stateGroups[groupId];
					dropdown.addOption(groupId, group.name);
				});
	...
	refreshStateGroupsList(container) {
		...
		Object.entries(this.plugin.settings.stateGroups).forEach(([groupId, groupData]) => {
			const group = this.plugin.settings.getStateGroup(groupId);
	...
	refreshPeopleList(container) {
		...
		Object.entries(this.plugin.settings.people).forEach(([personId, personData]) => {
			const person = this.plugin.settings.getPerson(personId);
	...
	/**
	 * Show modal to add new state group
	 */
	showAddStateGroupModal() {
		const modal = new StateGroupModal(this.app, this.plugin);
		modal.open();
	}

	/**
	 * Show modal to edit state group
	 * @param {TaskStateGroup} group - Group to edit
	 */
	showEditStateGroupModal(group) {
		const modal = new StateGroupModal(this.app, this.plugin, group);
		modal.open();
	}
