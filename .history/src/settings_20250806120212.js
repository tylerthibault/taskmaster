 ...
 * Uses plain objects for JSON serialization compatibility
 */
export class TaskMasterSettings {
	...
	getStateGroup(groupId) {
		const groupData = this.stateGroups[groupId];
		return groupData ? objectToStateGroup(groupData) : null;
	}
	 ...
	 */
	setStateGroup(group) {
		this.stateGroups[group.id] = stateGroupToObject(group);
	}
	...
	removeStateGroup(groupId) {
		delete this.stateGroups[groupId];
	}
	 ...
	 */
	getPerson(personId) {
		const personData = this.people[personId];
		return personData ? objectToPerson(personData) : null;
	}
	 ...
	 */
	setPerson(person) {
		this.people[person.id] = personToObject(person);
	}
	...
	/**
	 * Get all state groups as TaskStateGroup objects
	 * @returns {Array<TaskStateGroup>} Array of state groups
	 */
	getAllStateGroups() {
		return Object.values(this.stateGroups).map(groupData => objectToStateGroup(groupData));
	}

	/**
	 * Get all people as Person objects
	 * @returns {Array<Person>} Array of people
	 */
	getAllPeople() {
		return Object.values(this.people).map(personData => objectToPerson(personData));
	}
}

/**
 * Default settings with built-in state groups
	...
	stateGroups: {
		...
		'development': {
			...
			states: [
				...
				{ id: 'development', name: 'Development', color: '#f39c12', order: 2 },
				{ id: 'review', name: 'Code Review', color: '#9b59b6', order: 3 },
				{ id: 'testing', name: 'Testing', color: '#e67e22', order: 4 },
				{ id: 'deployed', name: 'Deployed', color: '#27ae60', order: 5 }
			]
		},
		'approval': {
			id: 'approval',
			...
			states: [
				{ id: 'draft', name: 'Draft', color: '#95a5a6', order: 0 },
				{ id: 'review', name: 'Under Review', color: '#3498db', order: 1 },
				{ id: 'feedback', name: 'Needs Feedback', color: '#e67e22', order: 2 },
				{ id: 'approved', name: 'Approved', color: '#27ae60', order: 3 },
				{ id: 'rejected', name: 'Rejected', color: '#e74c3c', order: 4 }
			]
		}
	},
	people: {},
