...
class LivePreviewHandler {
	 ...
	 */
	registerLivePreview() {
		this.logger.debug('Live preview handler ready');
		// For now, just rely on the main processor
		// Future enhancement: add real-time editing support
	}
...
class TaskMasterPlugin extends Plugin {
	async onload() {
		try {
			console.log('[TaskMaster] Starting plugin load...');
			...
			console.log('[TaskMaster] Live preview registered');

			this.commands.registerCommands(this);
			console.log('[TaskMaster] Commands registered');
			
			this.addSettingTab(new TaskMasterSettingTab(this.app, this));
			console.log('[TaskMaster] Settings tab added');
			
			this.addStyles();
			console.log('[TaskMaster] Styles added');

			this.logger.log('TaskMaster plugin loaded successfully');
			console.log('[TaskMaster] Plugin load complete!');
		} catch (error) {
			console.error('[TaskMaster] Error during plugin load:', error);
			throw error;
		}
	}
